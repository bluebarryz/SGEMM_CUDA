...
void run_naive_bad(int M, int N, int K, float alpha, float *A, float *B,
                   float beta, float *C) {
  // x dim -> row (M), y dim -> col (N)
  dim3 gridDim(CEIL_DIV(M, 32), CEIL_DIV(N, 32));
  dim3 blockDim(32, 32);
  naive_gemm_bad<<<gridDim, blockDim>>>(M, N, K, alpha, A, B, beta, C);
}

void run_naive_better(int M, int N, int K, float alpha, float *A, float *B,
                      float beta, float *C) {
  // x dim -> col (N), y dim -> row (M)
  dim3 gridDim(CEIL_DIV(N, 32), CEIL_DIV(M, 32));
  dim3 blockDim(32, 32);
  naive_gemm_better<<<gridDim, blockDim>>>(M, N, K, alpha, A, B, beta, C);
}
...
void run_kernel(int kernel_num, int M, int N, int K, float alpha, float *A,
  ...
  case 12:
    ...
    break;
  case 13:
    run_naive_bad(M, N, K, alpha, A, B, beta, C);
    break;
  case 14:
  ...
  case 15:
    ...
    break;
  case 16:
    run_1d_blocktiling(M, N, K, alpha, A, B, beta, C);
    break;
  default:
